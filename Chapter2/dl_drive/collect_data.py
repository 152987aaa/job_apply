...
# 运行2000次动作
pic_index = len(os.listdir(folder))
for t in range(2000):
    # 高斯滤波去噪
    frame_g = cv2.GaussianBlur(frame, (5, 5), 1)

    # 转换图像到HSV空间
    height, width, _ = frame_g.shape
    hsv = cv2.cvtColor(frame_g, cv2.COLOR_RGB2HSV)
    ...
    # 记录图像和转向角
    img_path = folder + "/{:d}_{:.4f}.jpg".format(pic_index, steering_angle)
